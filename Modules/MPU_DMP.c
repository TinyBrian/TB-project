...
extern Raw_UVAdata    Remote_data_raw;//remote raw data
...
{		//float Pitch,Roll,Yaw;
    ...
    int8_t accuracy;
		...
		if (inv_get_sensor_type_euler(data, &accuracy,(inv_time_t*)&timestamp))
			{	
				Measure_data.pitch =data[0]*1.0/(1<<16) ;

				Measure_data.roll = data[1]*1.0/(1<<16);
	
				Measure_data.yaw = data[2]*1.0/(1<<16);	

				ANO_DT_Data_Exchange();
															
			//	Measure_data.Battery=ADC_filter_return_battery();
				UVA_Controler(Remote_data_raw,Measure_data,(int16_t *)&sensors.gyro.raw); 					
			} 
								
}
	

static inline void run_self_test()
{
    int result;
    long gyro[3], accel[3];
...
retry:	
	result = mpu_init(&int_param); //MPU6050_INIT
	 if (result)//result=-1      
			{
		...
		{
			MPL_LOGE("initialized gyro.result =  %d\n",result);	
		}
    ...
    inv_enable_quaternion(); /* Compute 6-axis and 9-axis quaternions. */
	...
	if (result == INV_ERROR_NOT_AUTHORIZED) 
      while (1) { MPL_LOGE("Not authorized.\n");}
...
/****/ 
  ...
  get_tick_count(&timestamp);
   ...
   dmp_register_tap_cb(tap_cb);
     ...
     */
    hal.dmp_features = DMP_FEATURE_6X_LP_QUAT | DMP_FEATURE_TAP |DMP_FEATURE_SEND_RAW_ACCEL |
											 DMP_FEATURE_SEND_CAL_GYRO |DMP_FEATURE_GYRO_CAL;
  
		dmp_enable_feature(hal.dmp_features);
    dmp_set_fifo_rate(DEFAULT_MPU_HZ); //10 20 40 50 100
		run_self_test();
    mpu_set_dmp_state(1);
    hal.dmp_on = 1;
